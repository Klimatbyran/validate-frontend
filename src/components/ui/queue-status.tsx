...
export function QueueStatus() {
  const { totals, queuesStats, isLoading, isError, error, refresh } = useQueuesStats(10);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0">
          {queuesStats.map((queue, index) => {
            ...
            const status = stats.failed && stats.failed > 0 ? 'failed' : 
                          stats.active && stats.active > 0 ? 'active' : 
                          'completed';
            ...
            const isLast = index === queuesStats.length - 1;
            const isTopRow = index < 3;
            const isBottomRow = index >= queuesStats.length - 3;
            ...
            if (index === queuesStats.length - 3 && isBottomRow) roundedCorners = 'rounded-bl-[20px]';
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className={`text-base ${textColors[status]}/70`}>
                        {(stats.active || 0) + (stats.waiting || 0)} jobs in queue
                      </p>
                    </div>
                  </div>
                </div>
