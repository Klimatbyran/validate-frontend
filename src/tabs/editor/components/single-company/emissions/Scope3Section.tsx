...
import {
  ...
  emissionsPeriodEmptyCellClass,
} from "./EmissionsGridParts";

type PeriodColumn = {
  rp: GarboReportingPeriodSummary & { id: string };
  year: string;
};
...
export function Scope3Section({
  ...
  const undoField = (field: string) =>
    t("editor.periodEditor.undoField", { field });
  ...
  return (
    <>
      <EmissionsEditRow
        name={t("editor.periodEditor.scope3Section")}
        headerName
        noHover
      >
      ...
      <EmissionsEditRow
        name={
          <span className="ps-2">{t("editor.periodEditor.statedTotal")}</span>
        }
      >
        {visibleColumns.map(({ rp, year }) => {
          ...
          const verified =
            rpEdits.scope3StatedTotalVerified ?? originalVerified;
        ...
        <EmissionsEditRow
          ...
          name={
            <span className="ps-2 leading-snug">
              {scope3CategoryRowLabel(category)}
            </span>
          }
        >
          {visibleColumns.map(({ rp, year }) => {
            ...
            const value =
              editedVal ?? (originalVal != null ? String(originalVal) : "");
            ...
            const editedVerified =
              rpEdits.scope3CategoriesVerified?.[String(category)];
            const verified = hasEditedVerified
              ? !!editedVerified
              : originalVerified;
