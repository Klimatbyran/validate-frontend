...
function calcRates(dp: DataPointMetric) {
  ...
  const precisionTolerantRate =
    withAnyData > 0 ? ((identical + rounding) / withAnyData) * 100 : 0;
  const zeroInclusiveRate =
    totalSlots > 0 ? ((identical + rounding + bothNull) / totalSlots) * 100 : 0;
...
export function SummaryView({
  ...
  const rows = React.useMemo(() => {
    return allDataPointMetrics.map((dp) => {
      const { exactRate, precisionTolerantRate, zeroInclusiveRate } =
        calcRates(dp);
      return {
        id: dp.id,
        label: dp.label,
