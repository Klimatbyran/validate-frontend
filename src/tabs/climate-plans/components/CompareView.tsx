...
const CATEGORY_IDS: {
  ...
  {
    id: "targets",
    labelKey: "climate.compare.targets",
    icon: <Target size={16} />,
  },
  {
    id: "timeline",
    labelKey: "climate.compare.timeline",
    icon: <Calendar size={16} />,
  },
  {
    id: "framework",
    labelKey: "climate.compare.frameworks",
    icon: <Scale size={16} />,
  },
  { id: "scope", labelKey: "climate.compare.scope", icon: <Globe size={16} /> },
  {
    id: "accounting",
    labelKey: "climate.compare.accounting",
  ...
  if (all.size === 0)
    return (
      <span className="text-xs text-gray-02">
        {t("climate.compare.notSpecified")}
      </span>
    );
  return (
...
const ACCOUNTING_TYPE_KEYS: Record<
  string,
...
const FOCUS_KEYS: Record<
  string,
  {
    labelKey: string;
    badges: { labelKey: string; variant: "blue" | "green" | "orange" }[];
  }
> = {
  mitigation: {
...
const STRENGTH_KEYS: Record<
  string,
  { labelKey: string; variant: "green" | "blue" | "orange" | "default" }
> = {
...
function MunicipalitySummaryCard({
  m,
  onClick,
}: {
  ...
  const biggestReduction =
    reductionGoals.length > 0
      ? reductionGoals.reduce((max, c) =>
          Number(c.reduction_percentage) > Number(max.reduction_percentage)
            ? c
            : max,
        )
      : null;
  ...
  const frameworkFlags = [
    {
      val: et?.framework_alignment?.paris_agreement_mentioned,
      labelKey: "climate.compare.frameworkShortParis",
    },
    {
      val: et?.framework_alignment?.one_point_five_mentioned,
      labelKey: "climate.compare.frameworkShort1_5",
    },
    {
      val: et?.framework_alignment?.carbon_budget_referenced,
      labelKey: "climate.compare.frameworkShortCarbonBudget",
    },
    {
      val: et?.framework_alignment?.swedish_climate_act_referenced,
      labelKey: "climate.compare.frameworkShortSwedishClimateAct",
    },
    {
      val: et?.framework_alignment?.eu_frameworks_referenced,
      labelKey: "climate.compare.frameworkShortEu",
    },
  ];
  ...
  return (
    ...
    >
      ...
      {pt?.exists ? (
        <Callout
          variant="success"
          title={t("climate.compare.primaryTarget")}
          className="mb-3"
        >
          ...
          <Badge variant="default">
            {(pt.scope &&
              (scopeTKey[pt.scope]
                ? t(scopeTKey[pt.scope])
                : pt.scope.replace(/_/g, " "))) ||
              "—"}
          </Badge>
        </Callout>
      ) : (
        <div className="bg-gray-03/20 border border-gray-03/30 rounded-lg p-3 mb-3 flex flex-col justify-center min-h-[88px]">
          ...
          <div className="text-lg text-gray-02">
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        <div className="text-center">
          <div className="text-xl font-bold text-blue-03">
            {biggestReduction
              ? `${biggestReduction.reduction_percentage}%`
              : "—"}
          </div>
          <div className="text-xs text-gray-02 uppercase">
            {t("climate.compare.maxReduction")}
          </div>
        </div>
        <div className="text-center">
          <div className="text-xl font-bold text-gray-01">
            {ps?.temporal_scope?.plan_period_start || "—"}–
            {ps?.temporal_scope?.plan_period_end?.slice(-2) || "—"}
          </div>
          <div className="text-xs text-gray-02 uppercase">
            {t("climate.compare.planPeriod")}
          </div>
        </div>
        <div className="text-center">
      ...
      <div className="text-xs text-blue-03 mt-4">
        {t("climate.compare.viewFullDetails")}
      </div>
    </button>
  );
}
    ...
    >
      {municipalities.map((m) => {
        ...
        const reductionGoals = et.own_commitments.filter(
          (c) => c.goal_type === "emission_reduction",
        );
        const implementationGoals = et.own_commitments.filter(
          (c) => c.goal_type === "implementation",
        );
          ...
          <div key={m.id} className="space-y-4">
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {et.primary_target.confidence && (
                    <Badge
                      variant={
                        CONFIDENCE_STYLES[et.primary_target.confidence]
                          ?.variant || "default"
                      }
                    >
                      {et.primary_target.confidence}{" "}
                      {t("climate.compare.confidence")}
                    </Badge>
                  )}
                </div>
              </Callout>
            ...
            <div>
              <div className="text-xs text-gray-02 font-medium uppercase tracking-wider mb-2">
                ...
                <Badge variant={reductionGoals.length > 0 ? "blue" : "default"}>
            ...
            <div>
              <div className="text-xs text-gray-02 font-medium uppercase tracking-wider mb-2">
                ...
                <Badge
                  variant={
                    implementationGoals.length > 0 ? "orange" : "default"
                  }
                >
                  {implementationGoals.length}
                </Badge>
              </div>
              ...
              return (
                ...
                >
                  {yearEvents.map((e, i) => {
                    const color =
                      MUNICIPALITY_COLORS[
                        e.municipalityIndex % MUNICIPALITY_COLORS.length
                      ];
                    return (
                      <div
                        key={i}
                      ...
                      >
                        <span
                          className={cn("text-sm cursor-default", color.text)}
                        >
                          {typeIcons[e.type]}
                        </span>
                        {/* Tooltip */}
                  ...
                  <span className="text-xs text-gray-02 font-medium">
                    {year}
                  </span>
                </div>
              );
            ...
            <div key={m.id} className="space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {ts.has_revision_cycle && (
                  <Badge variant="green">
                    {t("climate.compare.hasRevisionCycle")}
                  </Badge>
                )}
                {!ts.has_revision_cycle && (
                  <Badge>{t("climate.compare.noRevisionCycle")}</Badge>
                )}
              </div>
...
function FrameworkPanel({
  ...
  const frameworks = [
    {
      key: "paris_agreement_mentioned" as const,
      labelKey: "climate.detail.parisAgreement",
    },
    {
      key: "one_point_five_mentioned" as const,
      labelKey: "climate.detail.onePointFiveTarget",
    },
    {
      key: "carbon_budget_referenced" as const,
      labelKey: "climate.detail.carbonBudget",
    },
    {
      key: "swedish_climate_act_referenced" as const,
      labelKey: "climate.detail.swedishClimateAct",
    },
    {
      key: "swedish_national_targets_referenced" as const,
      labelKey: "climate.compare.swedishNationalTargets",
    },
    {
      key: "eu_frameworks_referenced" as const,
      labelKey: "climate.detail.euFrameworks",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {municipalities.map((m) => (
          <div key={m.id}>
            <div className="text-sm font-medium text-gray-01 mb-3">
              {m.name}
            </div>
            <div className="flex flex-wrap gap-1.5">
              {frameworks.map((fw) => {
            ...
            {/* EU frameworks named */}
            {m.emissionTargets?.framework_alignment
              ?.eu_frameworks_referenced && (
              <div className="mt-2">
                <div className="text-xs text-gray-02 mb-1">
                  {t("climate.compare.euFrameworksLabel")}
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {m.emissionTargets.framework_alignment.eu_frameworks_named
                    ?.length > 0 ? (
                    m.emissionTargets.framework_alignment.eu_frameworks_named.map(
                      (name, i) => (
      ...
      <div>
        ...
        <div
          className="grid gap-6"
          style={{
            gridTemplateColumns: `repeat(${municipalities.length}, 1fr)`,
          }}
        >
          {municipalities.map((m) => {
                ...
                {refs && refs.length > 0 ? (
                  refs.map((r, i) => (
                    <div key={i} className="bg-gray-03/30 rounded-lg p-3">
                      <div className="text-sm text-gray-01 mb-1.5">
        ...
        return (
          <div key={m.id} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-02 shrink-0">
                  {t("climate.compare.spatialScope")}
                </span>
                <Badge variant="blue">
              ...
              <div className="flex items-center gap-3">
                ...
                <YesNoBadge
                  value={ss?.distinguishes_operations_vs_territory || false}
                />
              </div>

              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-02 shrink-0">
                  {t("climate.compare.regionalAlignment")}
                </span>
                <YesNoBadge
                  value={ss?.addresses_metropolitan_or_regional || false}
                />
              </div>
            </div>
          </div>
        ...
        return (
          <div key={m.id} className="space-y-4">
            ...
            <div>
              ...
              <ScopeBadges
                explicit={su?.ghg_scopes_explicit}
                implicit={su?.ghg_scopes_implicit}
              />
            </div>
...
function GoalCard({ c }: { c: OwnCommitment }) {
  ...
  return (
    <div className="bg-gray-03/30 rounded-lg p-3 mb-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        {hasReduction && (
          <Badge variant="blue">{c.reduction_percentage}%</Badge>
        )}
        ...
        {c.ghg_scopes_explicit?.length > 0 && (
          <Badge>
            {t("climate.compare.scopeNumber", {
              n: c.ghg_scopes_explicit.join("+"),
            })}
          </Badge>
        )}
        {c.ghg_scopes_implicit?.length > 0 && (
          <Badge>
            {t("climate.compare.scopeNumber", {
              n: c.ghg_scopes_implicit.join("+"),
            })}{" "}
            {t("climate.compare.implicit")}
          </Badge>
        )}
        {strength && (
          <Badge variant={strength.variant}>{t(strength.labelKey)}</Badge>
        )}
        {c.scope && (
          <Badge>
            {scopeTKey[c.scope]
