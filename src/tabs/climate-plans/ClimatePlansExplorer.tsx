...
import type {
  Measure,
  ActivityShift,
  TransitionElementCandidate,
  TransitionElementMatch,
  MatchConfidence,
...
function InterventionComponents({
  who,
  when,
  what,
  how,
        ...
        return (
          <div
            ...
            className={cn(
              ...
              present
                ? "bg-green-03/10 border-green-03/20"
                : "bg-gray-03/20 border-gray-03/30",
            )}
          >
            <span
              className={cn(
                "font-semibold mr-2",
                present ? "text-green-03" : "text-gray-02",
              )}
            >
              {label}
            </span>
...
function TaxonomyCandidate({
  ...
  candidate: TransitionElementCandidate;
  compact?: boolean;
}) {
  return (
    <div className="flex items-start gap-3 text-sm">
      ...
      <div className="min-w-0">
        <div className="text-gray-01 font-medium leading-snug">
          {candidate.short_label}
        </div>
        <div className="text-gray-02/60 leading-snug mt-0.5 truncate">
          {candidate.sector_path}
        </div>
        {!compact && candidate.description && (
          <div className="text-gray-02/80 leading-snug mt-0.5">
  ...
  return (
    <div className="flex items-start gap-3 text-sm">
      ...
      <div className="min-w-0">
        ...
        <div className="text-gray-02/60 leading-snug mt-0.5">
          {match.sector_path}
        </div>
        {match.description && (
          <div className="text-gray-02/80 leading-snug mt-0.5">
...
function sortedMatches(
  matches: TransitionElementMatch[],
): TransitionElementMatch[] {
  return [...matches].sort(
    (a, b) =>
      CONFIDENCE_ORDER[b.match_confidence] -
      CONFIDENCE_ORDER[a.match_confidence],
  );
}
  ...
  return (
    <div
      className={cn(
        ...
        expanded
          ? "border-gray-03/50 bg-gray-05/60"
          : "border-gray-03/30 bg-gray-05/40",
      )}
    >
      ...
      >
        <div className="flex-1 min-w-0 space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm font-semibold text-gray-01 capitalize">
              {shift.activity}
            </span>
            <span className="text-xs text-gray-02 bg-gray-03/30 rounded px-2 py-0.5">
              {shift.type}
            </span>
            {hasMatches && (
              <span className="text-xs text-green-03 bg-green-03/10 border border-green-03/20 rounded px-2 py-0.5">
                {shift.transition_element_matches.length} match
                {shift.transition_element_matches.length !== 1 ? "es" : ""}
              </span>
            )}
          </div>
        ...
        <div className="border-t border-gray-03/20 px-3 pb-3 pt-3 space-y-3">
          ...
          {shift.transition_element_suggested_new && (
            <div className="text-sm bg-orange-03/10 border border-orange-03/20 rounded-lg px-3 py-2">
              <span className="text-orange-03 font-semibold">
                Suggested new TE:{" "}
              </span>
              <span className="text-gray-01">
                {shift.transition_element_suggested_new.description}
              </span>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {hasMatches ? (
              sortedMatches(shift.transition_element_matches).map((m) => (
                <div
                  key={m.stable_id}
                  className="rounded-lg border border-gray-03/30 bg-gray-04/40 p-2.5"
                >
          ...
          {shift.transition_element_candidates.length > 0 && (
            <div className="space-y-2">
              ...
              >
                ...
                {shift.transition_element_candidates.length})
              </button>
              {showCandidates && (
                <div className="space-y-2">
...
function MeasureCard({
  ...
  const score =
    dimension === "activity"
      ? measure.activity_shift_score
      : measure.intervention_score;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            ...
            {dimension === "activity" && (
              <ActivityTag activity={measure.activity} />
            )}
            {dimension === "intervention" && (
              <InterventionTypeTag type={measure.intervention_type} />
            )}
          </div>
        </div>
      ...
      {counts.map((count, i) => (
        ...
        >
          <div
            ...
            style={{
              height: `${Math.max((count / max) * 32, count > 0 ? 4 : 0)}px`,
            }}
          />
...
function RankedList({
  ...
  const sorted = useMemo(() => {
    const key =
      dimension === "activity" ? "activity_shift_score" : "intervention_score";
    ...
    <SectionCard>
      ...
      <SectionCardBody className="p-4 space-y-3">
        {sorted.map((measure, i) => (
          <MeasureCard
            key={`${measure.measure_text.slice(0, 20)}-${i}`}
            measure={measure}
            dimension={dimension}
            rank={i + 1}
          />
    ...
    <div className="space-y-2">
      <div className="grid grid-cols-3 gap-3 text-sm">
        {[
          ...
          <div key={label}>
            <div className="text-gray-02 font-medium mb-0.5 text-xs uppercase tracking-wide">
              {label}
            </div>
            <div className="text-gray-01">{value || "—"}</div>
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span
          className={cn(
            "text-xs font-bold tabular-nums",
            scoreClasses(measure.intervention_score),
          )}
        >
      ...
      {(measure.intervention_who !== "none" ||
        ...
        <div className="flex gap-3 flex-wrap text-sm">
          {measure.intervention_who !== "none" && (
            <span>
              <span className="text-gray-02 font-medium">Who: </span>
              <span className="text-gray-01">{measure.intervention_who}</span>
            </span>
          )}
          {measure.intervention_what !== "none" && (
            <span>
              <span className="text-gray-02 font-medium">What: </span>
              <span className="text-gray-01">{measure.intervention_what}</span>
            </span>
          )}
          {measure.intervention_how !== "none" && (
            <span>
              <span className="text-gray-02 font-medium">How: </span>
              <span className="text-gray-01">{measure.intervention_how}</span>
            </span>
          )}
        </div>
      )}
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="text-base font-semibold text-gray-01">
            {group.element.short_label}
          </div>
          <div className="text-sm text-gray-02 mt-0.5">
            {group.element.sector_path}
          </div>
...
function TaxonomyView({ data }: { data: MunicipalityMeasures[] }) {
  ...
  const groups = useMemo(() => {
    ...
    return [...index.values()].sort((a, b) => {
      const muniDiff =
        uniqueMunicipalities(b.hits).length -
        uniqueMunicipalities(a.hits).length;
    ...
    <div className="space-y-5">
      ...
      {groups.length === 0 ? (
        <p className="text-sm text-gray-02 text-center py-16">
          No taxonomy matches yet. Populate{" "}
          <code className="font-mono text-xs">transition_element_matches</code>{" "}
          in your measures data to see cross-municipality connections here.
        </p>
      ) : filtered.length === 0 ? (
        <p className="text-sm text-gray-02 text-center py-10">
          No elements match your search.
        </p>
      ) : (
        <div className="space-y-3">
          {filtered.map((group) => (
            <TEGroupCard key={group.element.stable_id} group={group} />
          ))}
        </div>
      )}
    </div>
            ...
            {(["activity", "intervention"] as const).map((dim) => {
              const count = municipality.measures.filter(
                (m) =>
                  (dim === "activity"
                    ? m.activity_shift_score
                    : m.intervention_score) > 1,
              ).length;
              return (
                <div
                  key={dim}
                  className="bg-gray-04/80 rounded-lg border border-gray-03/30 px-5 py-4 space-y-3"
                >
