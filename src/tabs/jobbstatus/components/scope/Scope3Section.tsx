...
export function Scope3Section({
  ...
  const ourNumberMapForLatest = React.useMemo(
    () =>
      latestScope3Year
        ? buildOurNumberMapForYear(latestScope3Year, sortedScope3ByYear)
        : {},
    [sortedScope3ByYear, latestScope3Year],
  );
  const refNumberMapLatest = React.useMemo(
    () =>
      latestScope3Year
        ? buildRefNumberMap(latestScope3Year, referenceByYear)
        : {},
    [referenceByYear, latestScope3Year],
  );
  ...
  function renderReferenceSummary() {
    ...
    const snapshot = referenceByYear[latestScope3Year];
    if (!snapshot) return null;
    const yearEntry = sortedScope3ByYear.find(
      (entry) => entry.year === latestScope3Year,
    );
    ...
    return (
      <div className="mt-2 divide-y divide-gray-03">
        {categoryNumbers.map((categoryNumber) => {
          ...
          const label =
            SCOPE3_CATEGORY_NAMES[categoryNumber] ||
                ...
                <div className="font-extrabold text-gray-01 text-4xl mb-1 flex items-center gap-2">
                  ...
                  {hasReferenceForYear &&
                    (() => {
                      ...
                      return match ? (
                        <span className="text-green-03 text-2xl">✓</span>
                      ) : (
                        <span className="text-pink-03 text-2xl">✗</span>
                      );
                    })()}
                </div>
                <div className="text-base text-gray-02">
                  {normalized.unit || "tCO2e"}
                </div>
              </>
            ) : (
              <div className="text-base text-gray-02">
          ...
          {hasReferenceForYear ? (
            <div className="mt-4 divide-y divide-gray-03">
              {Array.from({ length: 15 }, (_, i) => i + 1).map((n) => {
                const our = buildOurNumberMapForYear(
                  entry.year,
                  sortedScope3ByYear,
                )[n];
                ...
                return (
                  ...
                  >
                    <span className="text-base text-gray-02">
                      {SCOPE3_CATEGORY_NAMES[n] ||
                        t("scope.categoryLabel", { n })}
                    </span>
                    <span className="font-extrabold text-gray-01 text-xl flex items-center gap-2">
                      {typeof our?.total === "number"
                        ? formatNumber(our!.total!)
                        : "—"}
                      {our?.unit ? ` ${our.unit}` : ""}
                      {match ? (
                        <span className="text-green-03">✓</span>
          ...
          <div className="bg-gray-04/80 border border-gray-03 rounded-xl p-4 border-l-4 border-l-orange-03">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-01">
                {t("scope.referenceValuesFromApi", { year: latestScope3Year })}
              </span>
            </div>
            {isLoading && (
            ...
            {!isLoading && !error && referenceByYear[latestScope3Year] && (
              <div className="space-y-2">
                ...
                {!referenceByYear[latestScope3Year]?.total &&
                  (!referenceByYear[latestScope3Year]?.categories ||
                    referenceByYear[latestScope3Year]!.categories!.length ===
                      0) && (
                    <div className="text-sm text-gray-02">
                      Inga referensvärden hittades för {latestScope3Year}.
                    </div>
                  )}
                {referenceJson && (
                  <details className="mt-3 bg-gray-04 rounded border border-gray-03 p-3">
                    <summary className="cursor-pointer text-sm font-medium text-orange-03">
                      Reference JSON ({latestScope3Year})
                    </summary>
                    <div className="flex justify-end">
                      <CopyButton
                        getText={() => JSON.stringify(referenceJson, null, 2)}
                      />
                    </div>
