...
function Scope1Card({
  ...
  const hasCombined =
    !!combinedScope1And2 && typeof combinedScope1And2.total === "number";

  if (!hasScope1 && !hasCombined) return null;

  const displayTotal = hasScope1 ? data!.total : combinedScope1And2!.total;
  const displayUnit =
    (hasScope1 ? data!.unit : combinedScope1And2!.unit) || "tCO2e";
  ...
  return (
    ...
    >
      ...
      <div className="text-base text-gray-02">{displayUnit}</div>
      ...
      {matchStatus !== "none" && (
        <div className="text-xs mt-2">
          {matchStatus === "match" ? (
            ...
            <span className="text-pink-03">
              ✗ {t("scope.deviatesFromProd")}
            </span>
          )}
        </div>
      )}
...
function Scope2Card({ data }: Scope2CardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="text-base text-gray-02 mt-2">{data.unit || "tCO2e"}</div>
      <div className="text-sm text-gray-02 mt-2">
...
export function Scope12Section({
  data,
  wikidataId,
      ...
      return (
        <div className="bg-gray-04/80 border border-gray-03 rounded-xl p-4 mb-4 border-l-4 border-l-green-03">
          ...
          {!isLoadingRef && refError && (
            <div className="text-sm text-gray-02">{refError}</div>
          )}
        </div>
      );
        ...
        {!isLoadingRef && !refError && (
          <div className="divide-y divide-gray-03">
            {ourS1 && (
              <div className="flex items-center justify-between py-1.5">
                ...
                <span className="text-base font-bold text-gray-01 flex items-center gap-2">
                  {snapshot.scope1?.total != null
                    ? formatNumber(snapshot.scope1.total)
                    : "—"}{" "}
                  {snapshot.scope1?.unit ?? ""}
                  {s1Match ? (
            ...
            {ourS2?.mb != null && (
              <div className="flex items-center justify-between py-1.5">
                <span className="text-sm text-gray-02">
                  {t("scope.scope2MarketBased")}
                </span>
                <span className="text-sm font-semibold text-gray-01 flex items-center gap-2">
                  {snapshot.scope2?.mb != null
                    ? formatNumber(snapshot.scope2.mb)
                    : "—"}{" "}
                  {snapshot.scope2?.unit ?? ""}
                  {s2MbMatch ? (
                    <span className="text-green-03">✓</span>
                  ) : (
                    <span className="text-pink-03">✗</span>
            ...
            {ourS2?.lb != null && (
              <div className="flex items-center justify-between py-1.5">
                <span className="text-sm text-gray-02">
                  {t("scope.scope2LocationBased")}
                </span>
                <span className="text-sm font-semibold text-gray-01 flex items-center gap-2">
                  {snapshot.scope2?.lb != null
                    ? formatNumber(snapshot.scope2.lb)
                    : "—"}{" "}
                  {snapshot.scope2?.unit ?? ""}
                  {s2LbMatch ? (
                    <span className="text-green-03">✓</span>
            ...
            {ourS2?.unknown != null && (
              <div className="flex items-center justify-between py-1.5">
                <span className="text-sm text-gray-02">
                  {t("scope.scope2Unspecified")}
                </span>
                <span className="text-sm font-semibold text-gray-01 flex items-center gap-2">
                  {snapshot.scope2?.unknown != null
                    ? formatNumber(snapshot.scope2.unknown)
                    : "—"}{" "}
                  {snapshot.scope2?.unit ?? ""}
                  {s2UnknownMatch ? (
                    <span className="text-green-03">✓</span>
            ...
            {ourCombined && (
              <div className="flex items-center justify-between py-1.5">
                <span className="text-sm text-gray-02">
                  {t("scope.scope12Total")}
                </span>
                <span className="text-sm font-semibold text-gray-01 flex items-center gap-2">
                  {snapshotCombined?.total != null
                    ? formatNumber(snapshotCombined.total)
                    : "—"}{" "}
              ...
              <div className="flex flex-col md:flex-row md:items-stretch justify-center gap-4 md:gap-0 mt-2 relative max-w-2xl mx-auto">
                {(() => {
                  ...
                  if (snapshot) {
                    if (
                      yearData.scope1 &&
                      typeof yearData.scope1.total === "number"
                    ) {
                      ...
                      yearData.scope1And2 &&
                      typeof yearData.scope1And2.total === "number"
                    ) {
                      const prodCombined = snapshot.scope1And2;
                      if (
                        prodCombined &&
                        typeof prodCombined.total === "number"
                      ) {
                        matchStatus =
