...
import { Job, Pipeline, PipelineNode } from "./types";

export function getInitialNode(pipeline: Pipeline) {
  const noneEndNodes = pipeline.filter(
    (node) => node.next && node.next.target.length > 0
  );
  for (const node of noneEndNodes) {
    if (
      noneEndNodes.find((n) => n.next?.target.includes(node.id)) === undefined
    ) {
      return node;
    }
  }
...
export function buildPipelineGraph(
  ...
  for (const pipelineNode of pipeline) {
    const queueJobs = jobs?.filter((jobI) => jobI.queue === pipelineNode.id);
    let jobData = {};

    if (queueJobs && queueJobs?.length <= 1) {
      jobData = {
        jobId: queueJobs[0]?.id,
        status: getNodeStatus(pipelineNode, pipeline, jobs),
      };
    } else if (queueJobs && queueJobs.length > 1) {
      const jobsData = queueJobs.map((job) => {
        return {
          jobId: job.id,
          status: job.status,
        };
    ...
    const node: Node = {
      ...
      style: {
        width: 180,
        height: queueJobs && queueJobs?.length > 1 ? "fit-content" : 50
      },
      type: queueJobs && queueJobs?.length > 1 ? "multi" : "custom",
      draggable: false,
    ...
    if (
      pipelineNode.next === undefined ||
      pipelineNode.next?.target.length === 0
    ) {
...
export function buildPipelineSubgraph(
  pipeline: Pipeline,
  node: PipelineNode
): Edge[] {
  const graph: Edge[] = [];
  if (node.next) {
    for (const target of node.next.target) {
      ...
      graph.push({
        ...
        target: nextNode.id,
        type:
          node.id === "nlmParsePDF" && nextNode.id === "precheck"
            ? "custom"
            : "default",
      });
    }
...
export function getPreviousNodes(
  ...
  for (const node of pipeline) {
    if (node.next?.target.includes(nodeId)) previousNodes.push(node);
  }
...
export function removeOrphanNodes(pipeline: Pipeline) {
  ...
  if (initialNode) cleanedPipeline.push(initialNode);
...
export function findFirstConvergenceNode(
...
export function getFollowingNodes(pipeline: Pipeline, startNodeId: string) {
  const nodeMap = new Map(pipeline.map((node) => [node.id, node]));
  const traverse = (nodeId: string, visited: Set<string>) => {
    ...
    node.next.target.forEach((targetId) => {
      if (!visited.has(targetId)) traverse(targetId, visited);
    });
  };

  const visited = new Set<string>();
  traverse(startNodeId, visited);
...
export function getNodeStatus(
  ...
  const job = jobs?.find((job) => job.queue === node.id);
  if (job) return job.status;

  const followingNodes = getFollowingNodes(pipeline, node.id);
  for (const followingNode of followingNodes) {
    const followingNodeJob = jobs?.find((job) => job.queue === followingNode);
    if (followingNodeJob) return "skipped";
  }
